...
def icmpv6_router_advertisement(source_mac,current_hop_limit,managed_address_configuration,other_configuration,reserved_field,router_lifetime,reachable_time,retrans_timer,myprefix,myprefixlength,router_priority,mymtu,interface):
	...
	if not mymtu:
		header=scapy.layers.inet6.ICMPv6ND_RA(code=0,chlim=current_hop_limit,M=managed_address_configuration,O=other_configuration,res=int(reserved_field),routerlifetime=router_lifetime,prf=router_priority,reachabletime=reachable_time,retranstimer=retrans_timer)/scapy.layers.inet6.ICMPv6NDOptSrcLLAddr(lladdr=mymac)/scapy.layers.inet6.ICMPv6NDOptPrefixInfo(prefixlen = myprefixlength,prefix = myprefix, validlifetime= 0xffffffff, preferredlifetime= 0xffffffff, L=1, R=1, A=1)   #M=0, O=0 implies that there is no information available via DHCPv6
	else:
		header=scapy.layers.inet6.ICMPv6ND_RA(code=0,chlim=current_hop_limit,M=managed_address_configuration,O=other_configuration,res=int(reserved_field),routerlifetime=router_lifetime,prf=router_priority,reachabletime=reachable_time,retranstimer=retrans_timer)/scapy.layers.inet6.ICMPv6NDOptSrcLLAddr(lladdr=mymac)/scapy.layers.inet6.ICMPv6NDOptMTU(mtu=mymtu)/scapy.layers.inet6.ICMPv6NDOptPrefixInfo(prefixlen = myprefixlength,prefix = myprefix, validlifetime= 0xffffffff, preferredlifetime= 0xffffffff, L=1, R=1, A=1)   #M=0, O=0 implies that there is no information available via DHCPv6
	return header
