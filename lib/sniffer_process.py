	...
	def handler(self,packets):
		...
		if packets.haslayer(IPv6):
			...
			if self.scan_type==1:
				...
				if packets.payload.haslayer(ICMPv6MLDone):
				...
				if multicast_address=="ff02::1:3":
					...
					res.append(packets.sprintf("/%ICMPv6MLReport.mladdr%/"))
				if packets.payload.haslayer(ICMPv6MLQuery):
				...
				if packets.haslayer(TCP) and (self.scan_type==3 or self.scan_type==1):
				...
				if packets.haslayer(UDP) and (self.scan_type==4 or self.scan_type==1 or self.scan_type==8): #8 is for DHCPv6 operation
